            ...
            <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-12 d-flex justify-content-end">
                        ...
                        <a href="{{ route('admin.master-status-operasi.create') }}" class="add-btn">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            @foreach($dataOk as $key => $item)
                                <tr>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td class="status-text" data-status="{{ $item->status_operasi }}">{{ $item->status_operasi }}</td> <!-- Menampilkan status_operasi -->
                                    <td>{{ $item->squence_status_operasi }}</td> <!-- Menampilkan squence_status_operasi -->
                                    <td>
                                            <a href="{{ route('admin.master-status-operasi.edit', $item->id) }}" class="reorder-btn btn-sm">
...
$(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('admin.master-status-operasi.index') }}" + '/' + id ,
                    type: 'DELETE',
                    data: {
                        "id": id,
                        "_token": '{{ csrf_token() }}'
